package com.coffee.management.payment.infrastructure.security;

import com.coffee.management.payment.application.Actor;
...
public class JwtVerifier {
    ...
    @Autowired
    public JwtVerifier(@Value("${payment.jwt.issuer}") String issuer,
            @Value("${payment.jwt.audience}") String audience,
            @Value("${payment.jwt.jwks-uri}") String jwksUri,
            @Value("${payment.jwt.jwks-cache-ttl:PT5M}") Duration cacheTtl) {
