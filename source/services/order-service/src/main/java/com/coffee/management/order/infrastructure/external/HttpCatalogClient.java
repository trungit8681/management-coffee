...
public class HttpCatalogClient implements CatalogPort {
    ...
    public Sellable sellable(UUID variantId, UUID branchId, String channel, String bearer) {
        try {
            var uri=base.resolve("api/v1/catalog/variants/"+variantId+"/sellable?branchId="+branchId+"&channel="+channel);
            var request=HttpRequest.newBuilder(uri).timeout(Duration.ofSeconds(4)).header("Authorization", bearer).GET().build();
            var response=http.send(request,HttpResponse.BodyHandlers.ofString());
            if (response.statusCode()==404) throw new OrderException(409,"ITEM_NOT_SELLABLE","Item unavailable");
            if (response.statusCode()!=200) throw new OrderException(503,"CATALOG_UNAVAILABLE","Cannot verify menu price");
            return mapper.readValue(response.body(),Sellable.class);
        } catch (OrderException ex) { throw ex; }
        catch (Exception ex) { throw new OrderException(503,"CATALOG_UNAVAILABLE","Cannot verify menu price"); }
    }
}
