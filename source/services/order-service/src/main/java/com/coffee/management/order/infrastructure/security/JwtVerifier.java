package com.coffee.management.order.infrastructure.security;

import com.coffee.management.order.application.Actor;
...
public class JwtVerifier {
    ...
    @Autowired
    public JwtVerifier(@Value("${order.jwt.issuer}") String issuer,
            @Value("${order.jwt.audience}") String audience,
            @Value("${order.jwt.jwks-uri}") String jwksUri,
            @Value("${order.jwt.jwks-cache-ttl:PT5M}") Duration cacheTtl) {
