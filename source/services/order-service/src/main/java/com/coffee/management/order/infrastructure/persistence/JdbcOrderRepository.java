...
public class JdbcOrderRepository {
    ...
    public Snapshot create(Order order, List<PricedItem> items, UUID actorId, String key, String hash) {
        ...
        int inserted = db.update(
                "INSERT INTO customer_order(id,branch_id,channel,status,payment_status,total_vnd,actor_id,idempotency_key,request_hash) VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT DO NOTHING",
                order.id(), order.branchId(), order.channel(), order.status(), paymentStatus, order.totalVnd(), actorId,
                key, hash);
        if (inserted == 0) {
        ...
        for (PricedItem priced : items) {
            ...
            db.update(
                    "INSERT INTO order_item(id,order_id,variant_id,quantity,unit_price_vnd,price_version,line_total_vnd) VALUES (?,?,?,?,?,?,?)",
                    UUID.randomUUID(), order.id(), item.variantId(), item.quantity(), item.unitPriceVnd(),
                    priced.priceVersion(), item.lineTotalVnd());
        }
        db.update(
