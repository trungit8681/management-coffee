...
public class OrderController {
    ...
    public record CreateRequest(@NotNull UUID branchId, @NotBlank String channel,
    ...
    public JdbcOrderRepository.Snapshot create(@Valid @RequestBody CreateRequest body,
        ...
        return service.create(new OrderApplicationService.Create(body.branchId(), body.channel(),
                body.items().stream().map(i -> new OrderApplicationService.RequestedItem(i.variantId(), i.quantity()))
                        .toList()),
                key, actor, bearer);
    }

    @GetMapping("/{id}")
    public JdbcOrderRepository.Snapshot get(@PathVariable UUID id, @AuthenticationPrincipal Actor actor) {
