...
public class PaymentApplicationService {
    ...
    public PaymentApplicationService(OrderPort orders, JdbcPaymentRepository repository, TransactionTemplate transactions) {
        this.orders = orders; this.repository = repository; this.transactions = transactions;
    }

    public JdbcPaymentRepository.Receipt collect(UUID orderId, long receivedVnd, String key, Actor actor, String bearer) {
        ...
        return transactions.execute(status -> repository.record(payment, key, hash(payment, actor.userId()), actor.userId()));
    }
    public JdbcPaymentRepository.RefundReceipt refund(UUID orderId,String reason,String key,Actor actor,String bearer) {
        if (key==null || key.isBlank() || key.length()>128) throw new PaymentException(400,"INVALID_IDEMPOTENCY_KEY","Idempotency-Key required");
        if (reason==null || reason.isBlank() || reason.length()>500) throw new PaymentException(400,"INVALID_REASON","Refund reason required");
        var replay=repository.replayRefund(key,orderId,reason,actor.userId());
        if (replay!=null) { actor.require("payment:refund_cash",replay.branchId()); return replay; }
        ...
        return transactions.execute(status -> repository.recordRefund(refund,key,refundHash(refund,actor.userId()),actor.userId()));
    }
    public static String refundHash(CashRefund refund,UUID actorId) {
        try {
            String input=refund.orderId()+":"+refund.branchId()+":"+refund.amountVnd()+":"+refund.reason()+":"+actorId;
    ...
    public static String hash(CashPayment payment, UUID actorId) {
        try {
            String input = payment.orderId() + ":" + payment.branchId() + ":" + payment.totalVnd() + ":" + payment.receivedVnd() + ":" + actorId;
            return HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(input.getBytes(StandardCharsets.UTF_8)));
        } catch (Exception ex) { throw new IllegalStateException(ex); }
    }
}
